...
#[embassy_executor::task]
async fn uart_rx_process(mut rx: Uart<'static, Async>, mut buf: &'static mut [u8; BUF_SIZE]) {
    loop{

            if let Ok(n) = rx.read_until_idle(buf).await {
...
#[embassy_executor::task]
async fn uart_tx_process(mut tx: Uart<'static, Async>, mut buf: &'static mut [u8; BUF_SIZE]) {
    loop{
        match select(
            tx.write_until_idle(buf),
            Eth_to_uart.receive(),
        ).await {
            ...
            Either::First(result) => {
                if let Ok(n) = result {
                        Eth_to_uart.send(buf[..n]).await;
                    }
                }
            Either::Second(data) => {
                if let Err(e) = rx.write_all(data).await {
