...
//
//#[embassy_executor::task]
//async fn uart_read_process(mut uart:Uart<'static, Async>){
//    let mut buf = [0; 4096];
//    loop{
...
async fn main(spawner: Spawner) {
    ...
    let mut usart = Uart::new(p.USART1, p.PA10, p.PA9, Irqs, p.DMA1_CH4, p.DMA1_CH5, config).unwrap();
    ...
    //unwrap!(spawner.spawn(uart_read_process(usart)));
    ...
    loop {
        ...
        'read:loop {
            match select(socket.read(&mut ebuf), usart.read_until_idle(&mut ubuf)).await {
                Either::First(result) => {
                    match result {
                        Ok(0) => {
                            warn!("read EOF");
                        ...
                        Ok(n) => {
                            info!("get eth data:{:?}",n);
                            let _ = usart.write(&ebuf[..n]).await;
                        },
                        Err(e) => {
                            warn!("read error: {:?}", e);
				break 'read;
                ...
                Either::Second(result) => {
                    match result {
                        Ok(n) => {
                            warn!("uart read: {:?}",n);
                            let _ = socket.write_all(&ubuf[..n]).await;
                        }
                        Err(e) => {
